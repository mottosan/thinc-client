...
/**
 * ThincSMessageFillGlyph. Fills an area at coordinates x,y of n-width and
 * n-height with a transparent bitmap.
 */

final class ThincSMessageFillGlyph extends ThincSMessageBase {
	public static Logger logger = ThincLogger
			.getLogger(ThincSMessageFillGlyph.class);
	// static constants
	/** Value of this is "16" */
	static final byte MESSAGE_TYPE = 16;
	...
	ThincSMessageFillGlyph(ThincProtocol proto) {
	...
	public void processServerMessage(SocketChannel sc) throws ThincException {
		logger
				.debug("Entering processServerMessage within ThinSMessageFillGlyph");
		ThincProtocol.debug("Got FillGlyph");
		...
		// :HACK: need to mask the alpha channel for this color
		// otherwise, the color appears inverted
		color = bb.getInt() | 0xffffffff;
		// color=canvas.argbToAbgr(color);
		...
		int[] pixels = new int[Math.abs(w * h)];
		...
		ThincClientCanvas.generateBitMapImage(updateArray, pixels, color,
				0x00000000, w, bitMapSizeInBytes);
		...
		bb.clear();
		logger
				.debug("Exiting processServerMessage within ThinSMessageFillGlyph");
	}
