...
import java.util.zip.InflaterInputStream; // Added by Nani: 11/14/2005
...
public class PNGDecoder {
	...
	public static BufferedImage decode(ByteBuffer in) throws IOException {
		readSignature(in);
	...
	protected static void readSignature(ByteBuffer in) throws IOException {
		long signature = in.getLong();
		if (signature != 0x89504e470d0a1a0aL)
			throw new IOException("PNG signature not found!");
	}

	protected static PNGData readChunks(ByteBuffer in) throws IOException {
		...
		while (in.hasRemaining()) {
			try {
				...
				// Read the CRC.
				long crc = in.getInt() & 0x00000000ffffffffL; // Make it
				// unsigned.
				...
				throw new IOException("error reading Chunks");
			}
		}
		return chunks;
	}
